<script lang="ts">
	...
	function applyProfile(value: Profile) {
		profile = value; displayName = value.display_name || ''; bio = value.bio || ''; website = value.website || ''; country = value.country || ''; timezone = value.timezone || value.buyer?.timezone || 'Asia/Hong_Kong';
	...
	async function saveProfile() {
		saving = true; error = '';
		try {
			await api('profile', { method: 'PATCH', body: JSON.stringify({ displayName, bio, website, country, timezone, buyer: { legalName, companyName, roleTitle, country, timezone, budgetRange: buyerBudget, purchaseEntity } }) });
