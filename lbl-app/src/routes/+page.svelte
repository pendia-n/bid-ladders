<script lang="ts">
	...
	let registrationTotpEnabled = $state(false);
	...
	async function openAuth(mode: 'login' | 'signup') {
		...
		if (mode === 'login') { registrationTotp = null; registrationTotpQr = ''; registrationTotpCode = ''; registrationTotpEnabled = false; }
		modal = 'auth';
	}

	async function toggleRegistrationTotp() { registrationTotpEnabled = !registrationTotpEnabled; if (registrationTotpEnabled && !registrationTotp) { const secret = registrationSecret(); const otpauth = `otpauth://totp/BidLadders:new-account?secret=${secret}&issuer=BidLadders`; registrationTotp = { secret, otpauth }; registrationTotpQr = await QRCode.toDataURL(otpauth, { width: 220, margin: 2 }); } }
	...
	async function loadStripeKeyStatus() {
		stripeKeyStatus = '';
		try { const data = await api('stripe/connection'); stripeKeyStatus = data.maskedKey || ''; } catch { stripeKeyStatus = ''; }
	}

	async function openEditListing(listing: Listing) {
		...
		modal = 'listing'; error = ''; await loadStripeKeyStatus();
	}
	...
	async function submitAuth() {
		...
		try {
			...
			const payload = authTotpRequired ? { userId: authUserId, code: authCode } : { username: authUsername, password: authPassword, role: authRole, totpSecret: authMode === 'signup' && registrationTotpEnabled ? registrationTotp?.secret : '', totpCode: authMode === 'signup' && registrationTotpEnabled ? registrationTotpCode : '' };
	...
	<div class="modal-backdrop" role="presentation" onclick={(event) => event.currentTarget === event.target && (modal = null)}>
		<div class="modal" role="dialog" aria-modal="true">
			...
			{:else if modal === 'listing'}
				...
				<div class="field stripe-key-row"><label for="stripe-key">Restricted Stripe key (optional)</label>{#if editingListing && stripeKeyStatus}<div class="saved-secret" aria-live="polite"><span>Saved connection</span><strong>{stripeKeyStatus}</strong><small>The full key stays encrypted on the server. Leave the field below blank to reuse it.</small></div>{/if}<div class="input-action"><input id="stripe-key" type="password" bind:value={stripeKey} placeholder={stripeKeyStatus ? 'Leave blank to reuse saved key' : 'rk_test_...'} /><a class="icon-button" href={stripeCreateUrl} target="_blank" rel="noreferrer" title="Open the prefilled Stripe restricted-key form in a new tab" aria-label="Open the prefilled Stripe restricted-key form in a new tab">↗</a></div><small class="field-help">Enter a key only when adding or replacing the connection. The same saved key can verify every product in this Stripe account.</small></div>
